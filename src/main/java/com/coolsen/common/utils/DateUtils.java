...
//            }
    ...
    public static String getShortTime(Date date) {
        if (Objects.isNull(date)) throw new IllegalArgumentException("date invalid!");
        ...
        if (delTime > 31536000L) {
            // 1年前（yyyy年MM月dd日 HH:mm）
            shortString = calendar.get(1) + "年" + integerTo(calendar.get(2) + 1) + "月" + integerTo(calendar.get(5)) + "日 " + integerTo(calendar.get(11)) + ":" + integerTo(calendar.get(12));
        } else if (delTime > 86400L) {
            // 1天前（24小时前）
            if ((int) (delTime / 86400L) <= 1) {
                // 1天前2天内（24小时前 小于48小时）
                shortString = "昨天 " + integerTo(calendar.get(11)) + ":" + integerTo(calendar.get(12));
            } else if ((int) (delTime / 86400L) <= 2) {
                // 2天前3天内
                shortString = "前天 " + integerTo(calendar.get(11)) + ":" + integerTo(calendar.get(12));
            } else {
                // 3天前1年内
                shortString = integerTo(calendar.get(2) + 1) + "月" + integerTo(calendar.get(5)) + "日 " + integerTo(calendar.get(11)) + ":" + integerTo(calendar.get(12));
            }
            ...
            // 1分钟前
            shortString = integerTo((int) (delTime / 60L)) + "分前";
        } else {
            // 1分钟内
            shortString = "刚刚";
        }
